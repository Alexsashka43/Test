...
def update_date():
    ...
    f_documents = os.path.join(current_path, 'fixtures/document.json')
...
def remove_doc_from_shelf(doc_number):
    for documenty_number, directory_docs_list in directories.items():
...
def get_doc_shelf():
    user_doc_number = input('Введинет номер документа - ')
    doc_exist = check_document_existance(user_doc_number)
    if doc_exist:
...
def secretary_program_start():
    """
    ap - all people - команда, которая выводит спискок всех владельцев документов
    ...
    while True:
        ...
        if user_command == 'p':
            ...
            new_doc_shelf_number = add_new_shelf()
            ...
            shelf_number, added = add_new_doc()
            if added:
